...
- **Expo AI tooling reference** - Added `platform/expo/ai-tooling.md` covering the Expo MCP (`expo.dev/ai` - read dashboard build logs, control the simulator; server vs local capabilities), `expo doctor` for catching version mismatches before debugging, and the Expo skills registry (`expo.dev/skills`). Scoped to Expo-technical tooling; generic agent-workflow hygiene deliberately excluded.
